...
class SingleBeamSearchBoard():
  def __init__(self, device,prev_status_config,beam_size=5,max_length=255):
    ...
    self.cummulative_probs = [torch.FloatTensor([.0] + [-float('inf')] * (beam_size - 1)).to(self.device)]
    ...
    for prev_status_name, each_config in prev_status_config.items():
      ...
      batch_dim_index = each_config['batch_dim']
  ...
  def get_length_penalty(self, length, alpha=LENGTH_PENALTY, min_length=MIN_LENGTH):
    p = ((min_length + 1) / (min_length + min_length))**alpha
    ...
    for b in range(self.beam_size):
      if self.cumulative_probs[-1][b] != -float('inf'):
        if not (len(self.cumulative_probs) -1, b) in founds:
          ...
          founds += [(t,b)]
    sorted_founds_with_probs = sorted(zip(founds,probs), key=itemgetter(1), reverse=True)[:n]
    probs = []
